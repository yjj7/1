...
import { BookOpen, ArrowRight, BarChart3, Clock, Flame, Target } from 'lucide-react';
    ...
    <div className="relative min-h-screen w-full flex flex-col text-white overflow-hidden font-sans">
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-6">
        ...
        <nav className="flex items-center space-x-6 text-sm text-white/70">
          <button onClick={onStats} className="hover:text-white transition-colors flex items-center space-x-1.5">
            <BarChart3 className="w-4 h-4" /><span>统计</span>
          </button>
          <button onClick={onHistory} className="hover:text-white transition-colors flex items-center space-x-1.5">
            <Clock className="w-4 h-4" /><span>历史</span>
          </button>
        </nav>
      </header>
            ...
            <div className="flex items-center space-x-6 mb-8">
              {streak > 0 && (
                <div className="flex items-center space-x-2 px-4 py-2 rounded-full bg-orange-500/20 border border-orange-500/30">
                  ...
                  <span className="text-sm text-orange-300">连续 {streak} 天</span>
                </div>
              )}
              {pomodoroCount > 0 && (
                <div className="flex items-center space-x-2 px-4 py-2 rounded-full bg-green-500/20 border border-green-500/30">
                  <Target className="w-4 h-4 text-green-400" />
                  <span className="text-sm text-green-300">{pomodoroCount} 个番茄</span>
                </div>
              )}
            </div>
          )}
      ...
      <footer className="relative z-10 py-8 px-8 md:px-24 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center bg-gradient-to-t from-black/60 to-transparent">
        ...
        <div className="flex gap-16 text-sm">
          <div className="flex flex-col space-y-3 text-white/60">
            ...
            <button onClick={onStart} className="text-left hover:text-white transition-colors">场景</button>
            <button onClick={onStart} className="text-left hover:text-white transition-colors">音乐</button>
          </div>
          <div className="flex flex-col space-y-3 text-white/60">
            <span className="text-white font-medium mb-1">数据</span>
            <button onClick={onStats} className="text-left hover:text-white transition-colors">学习统计</button>
            <button onClick={onHistory} className="text-left hover:text-white transition-colors">学习历史</button>
          </div>
        </div>
      </footer>
    </div>

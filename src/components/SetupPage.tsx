...
import { BookOpen, Headphones, Volume2, ArrowRight, Upload, Music, Image, BarChart3, Clock, Target, Flame } from 'lucide-react';
...
export function SetupPage(props: SetupPageProps) {
  ...
  const handleBgUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      onCustomBgChange(URL.createObjectURL(file));
    }
  };
                ...
                return (
                  <button key={scene.id} onClick={() => onSelectScene(scene.id)}
                    ...
                    <div className="aspect-[16/10] w-full overflow-hidden relative">
                      ...
                      {isActive && <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white flex items-center justify-center">
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-xl bg-white/[0.03] backdrop-blur-sm border border-white/10">
                ...
                <select value={selectedMusicId} onChange={(e) => onSelectMusic(e.target.value)}
                ...
                {selectedMusicId === 'custom' && (
                  <div className="mt-3">
                    ...
                    <button onClick={() => musicFileRef.current?.click()}
                      ...
                      <Upload className="w-3.5 h-3.5" /><span>{customMusicUrl ? '已选择文件' : '上传本地音乐'}</span>
                    </button>
          ...
          <div>
            ...
            <div className="p-4 rounded-xl bg-white/[0.03] backdrop-blur-sm border border-white/10">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <span className="text-sm">每日目标 {dailyGoal.targetMinutes} 分钟</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {[60, 120, 180].map(min => (
                  <button key={min} onClick={() => onDailyGoalChange({ targetMinutes: min })}
                    className={`py-2 rounded-lg border text-xs transition-all ${dailyGoal.targetMinutes === min ? 'border-white bg-white/20 font-medium' : 'border-white/15 hover:bg-white/10'}`}>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {DURATIONS.map(dur => (
                  <button key={dur} onClick={() => { onTimerDurationChange(dur); (document.getElementById('custom-time') as HTMLInputElement) && ((document.getElementById('custom-time') as HTMLInputElement).value = ''); }}
                    className={`py-3 rounded-lg border text-sm transition-all ${timerDuration === dur ? 'border-white bg-white/20 font-medium' : 'border-white/15 hover:bg-white/10'}`}>
                    {dur} 分钟
                  </button>
                ))}
                <div className={`relative flex items-center rounded-lg border overflow-hidden ${![...DURATIONS].includes(timerDuration) ? 'bg-white/15 border-white' : 'border-white/15'}`}>
                  <input id="custom-time" type="number" placeholder="自定义" className="w-full bg-transparent text-center outline-none py-3 text-sm placeholder:text-white/30"
                    onChange={(e) => { const v = Number(e.target.value); if (v > 0) onTimerDurationChange(v); }} />
